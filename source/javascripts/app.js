...
function setCodeOverflow() {
    return;
    jQuery('pre.highlight').each(function() {
        jQuery(this).css({
            "overflow-x": "visible"
        });

        var gutterWidth = jQuery(this).find('.gutter').outerWidth();
        var tdWidth = jQuery(this).outerWidth() - gutterWidth - 2;
        jQuery(this).find('.code').css({width: tdWidth});
    });
}
